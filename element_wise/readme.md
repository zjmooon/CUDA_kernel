...
在现代 GPU 上：𝑂𝐼𝑟𝑖𝑑𝑔𝑒=𝑃𝑝𝑒𝑎𝑘 / 𝐵𝑊𝑝𝑒𝑎𝑘≈10∼20

而本 kernel 的 OI：
0.083 ≪ 𝑂𝐼𝑟𝑖𝑑𝑔𝑒

所以compute不是瓶颈，所有有效优化只能围绕memory。
...
**不是提高 Roofline 上限，而是更接近带宽 Roofline**
...
实际上：prefetch应用于 latency-bound

从ncu metric中也可以发现：
![alt text](ncu/Roofline.png)

以naive kernel为例，最里层的斜线表示DRAM的 memory roof，而kernel 正好落在这个斜线附近，也证明了它属于 memory-bound。

所以，没有必要再做prefetch优化。

只有点在斜线下方很远
才可能是 latency / occupancy / launch 问题。
（也可以说：当点距离折线较远时，需要做一些基础的优化）
...
1. float4 优化的本质是减少指令数，提高带宽利用率，而非提高算力
